...
sgs.ai_skill_use_func.TransferCard = function(transferCard, use, self)
	--self.room:writeToConsole("合纵连横判断开始:" ..self.player:objectName())
	...
	for _, c in sgs.qlist(self.player:getCards("h")) do
		if c:isTransferable() and (not isCard("Peach", c, self.player) or #friends_shown > 0) then
			...
			elseif self:hasCrossbowEffect() and c:isKindOf("Slash") then
				continue
			elseif sgs.isBigKingdom(self.player, "transfer") and c:isKindOf("ThreatenEmperor") then
				continue
	...
	if has_TE or has_BC then
			...
			if sgs.isBigKingdom(self.player, "transfer") then
				for _, card in ipairs(cards) do
					if card:isKindOf("ThreatenEmperor") then--大势力留下挟天子
						table.removeOne(cards, card)
						self.room:writeToConsole("大势力留下SOS社团活动")
					end
				end
				has_TE = false
			end
		if has_TE then--同时有火烧和挟天子时目标是否合适？
			--self.room:writeToConsole("合纵连横判断挟天子:" ..self.player:objectName())
			local anjiang = 0
		...
		elseif has_BC then
			--self.room:writeToConsole("合纵连横判断火烧连营:" ..self.player:objectName())
			...
			if string.find(gameProcess, self.player:getKingdom() .. ">>>") then--必须要>个数多的在前，因为只会find第一个>
				for _, card in ipairs(cards) do
					if card:isKindOf("BurningCamps") then--大国？火烧联营不能给
						table.removeOne(cards, card)
						self.room:writeToConsole(">>>大优势，天破壤碎不能给")
					end
				end
		...
		if target and #cards > 0 then
			--self.room:writeToConsole("合纵连横含重要卡牌对象:" .. target:objectName())
			for _, card in ipairs(cards) do
			...
			--self.room:writeToConsole("合纵连横含重要卡牌:" .. card_str)
			return
		end
	end
	...
	if #friends_shown > 0 then
		--self.room:writeToConsole("有明置的友方")
		...
		if #friends_shown > 0 then
			...
			if p then
				...
				--self.room:writeToConsole("合纵连横卡牌对象:" .. p:objectName())
				--self.room:writeToConsole("合纵连横卡牌:" .. card_str)
				return
			end
		end
	end
	...
	if #friends_other > 0 then
		--self.room:writeToConsole("有暗置的友方")
