        ...
        if (triggerEvent == CardUsed) {
            ...
            if (use.card) {
                foreach (ServerPlayer *p, use.to) {
                    if (p->hasFlag("blade_use")){
                        room->setPlayerFlag(p, "-blade_use");
...
bool ThreatenEmperor::isAvailable(const Player *player) const
{
    if (!player->hasShownOneGeneral())
        return false;
    //animemod new rule
    if (player->getMark("NewThreatenEmperorExtraTurn")>0)
        return false;
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (change.to != Player::NotActive)
            return list;
        if (player->getMark("NewThreatenEmperorExtraTurn-prevent") > 0){
            room->setPlayerMark(player, "NewThreatenEmperorExtraTurn-prevent", 0);
        }
        else if (player->getMark("NewThreatenEmperorExtraTurn") > 0){
            room->setPlayerMark(player, "NewThreatenEmperorExtraTurn", 0);
        }
        foreach(ServerPlayer *p, room->getAllPlayers()){
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *, QVariant &, ServerPlayer *ask_who) const
    {
        ...
        room->setPlayerMark(ask_who, "NewThreatenEmperorExtraTurn-prevent", 1);
        room->setPlayerMark(ask_who, "NewThreatenEmperorExtraTurn", 1); //animemod new rule
