...
public:
    ...
    virtual int getResidueNum(const Player *from, const Card *card) const
    {
        if (from->hasSkill(objectName())||from->property("touying_type").toString()==objectName())
            return 1000;
        else
            return 0;
    }
...
void StandardPackage::addShuGenerals()
{
    skills << new CrossbowSkill
           <<new Rende<<new Wusheng<< new Paoxiao
          <<new Guanxing << new Kongcheng << new Longdan
         <<new Mashu("machao")<<new Tieqi
        << new Jizhi<<new Qicai<<new Liegong<< new LiegongRange
         <<new Kuanggu <<new Lianhuan<<new Niepan
        << new Bazhen<<new Huoji<<new Kanpo
        <<new Xiangle<< new Fangquan << new Huoshou<< new Zaiqi
       <<new Lieren<<new Juxiang<< new Shushen<< new Shenzhi;
