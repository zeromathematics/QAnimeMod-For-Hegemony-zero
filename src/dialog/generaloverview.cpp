...
QWidget *GeneralSearch::createInfoTab()
{
    ...
    anime_label = new QLabel(tr("Anime"));
    anime_label->setToolTip(tr("<font color=%1>Input characters included by the anime. '?' and '*' is available. Every anime meets the condition if the line is empty.</font>").arg(Config.SkillDescriptionInToolTipColor.name()));
    anime_edit = new QLineEdit;
    anime_edit->clear();
    layout->addLayout(HLay(anime_label, anime_edit));
...
void GeneralOverview::on_tableView_clicked(const QModelIndex &index)
{
    ...
    QString skill_description = general->getSkillDescription(true, false, true);
#ifdef Q_OS_ANDROID
...
{
    startSearch(detail.include_hidden, detail.anime, detail.nickname, detail.name, detail.genders, detail.kingdoms, detail.lower, detail.upper, detail.packages);
}

void GeneralOverview::startSearch(bool include_hidden, const QString &anime, const QString &nickname, const QString &name, const QStringList &genders, const QStringList &kingdoms, int lower, int upper, const QStringList &packages)
{
