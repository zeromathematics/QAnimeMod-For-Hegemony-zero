...
void JiangeDefenseScenario::assign(QStringList &generals, QStringList &generals2, QStringList &kingdoms, Room *room) const
{
    ...
    roles.insert("science", wei_roles);
    roles.insert("magic", shu_roles);
    qShuffle(kingdoms);
...
//                human_map.insert(players[i], answer);
                human_map.insert(players[i], "science");
...
//                }
//                answer.prepend("magic");
//                human_map.insert(players[i], answer);
                human_map.insert(players[i], "magic");
            }
        }
    }
...
//            kingdoms << answer.takeFirst();
            ...
            if (role == "ghost") {
                QString name = kingdom == "science" ? getRandomWeiGhost() : getRandomShuGhost();
                generals << name;
                generals2 << name;
            } else if (role == "machine") {
                QString name = kingdom == "science" ? getRandomWeiMachine() : getRandomShuMachine();
                generals << name;
                generals2 << name;
            } else if (role == "human") {
                int n = kingdom == "science" ? 4 : 5;
                QStringList choices;
                for (int j = 0; j < n; j++)
                    choices << ((kingdom == "science") ? wei_generals.takeFirst() : shu_generals.takeFirst());
