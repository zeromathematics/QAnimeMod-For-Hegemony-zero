...
QString General::getSkillDescription(bool include_name, bool inToolTip, bool include_related) const
{
    ...
    //Lines
    QString  line = Sanguosha->translate("%"+objectName());
    if (line != "%"+objectName() ){
            description.append(QString("<font color=%1><b>%2</b>").arg("#1E90FF").arg(line));
...
bool General::isCompanionWith(const QString &name) const
{
    ...
    bool same = other->getKingdom() == getKingdom()? true : false;
    Q_ASSERT(other);
    return (lord && same) || (other->lord && same) || companions.contains(name)
