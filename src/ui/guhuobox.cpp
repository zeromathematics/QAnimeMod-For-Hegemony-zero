...
void GuhuoBox::popup()
{
    ...
    if (flags.contains("s")) {
        foreach (const Card *card, cards) {
            if (card->isNDTrick() && !ServerInfo.Extensions.contains("!" + card->getPackage())) {
                if (names5.contains(card->objectName()) || !isButtonEnable(card->objectName()))
                    continue;

                if (card->inherits("SingleTargetTrick"))
                    names5 << card->objectName();
            }
        }
        if (!names5.isEmpty()) {
            Title *newtitle = new Title(this, translate("SingleTargetTrick"), IQSanComponentSkin::QSanSimpleTextFont::_m_fontBank.key(G_COMMON_LAYOUT.graphicsBoxTitleFont.m_fontFace), Config.TinyFont.pixelSize());
            newtitle->setParentItem(this);
            titles << newtitle;
            maxcardcount = qMax(maxcardcount, names5.length());
            ++maxrow;
        }
    }
    if (flags.contains("e")) {
    ...
    if (flags.contains("k")) {
        foreach (const Card *card, cards){
            if (card->isNDTrick() && !ServerInfo.Extensions.contains("!" + card->getPackage())) {
                ...
                if (card->inherits("SingleTargetTrick"))
                    names7 << card->objectName();
                else
                    names8 << card->objectName();
        ...
        if (!names7.isEmpty()) {
            ...
            newtitle->setParentItem(this);
            titles << newtitle;
            maxcardcount = qMax(maxcardcount, names7.length());
            ++maxrow;
        }
        if (!names8.isEmpty()) {
            Title *newtitle = new Title(this, translate("MultiTarget"), IQSanComponentSkin::QSanSimpleTextFont::_m_fontBank.key(G_COMMON_LAYOUT.graphicsBoxTitleFont.m_fontFace), Config.TinyFont.pixelSize());
            newtitle->setParentItem(this);
            titles << newtitle;
            maxcardcount = qMax(maxcardcount, names8.length());
            ++maxrow;
        }
        foreach (const Card *card, cards) {
            if (Self->getMark(card->objectName()+"zhanshu")==0&&!card->isNDTrick() && card->isKindOf("TrickCard") && !names9.contains(card->objectName()) && isButtonEnable(card->objectName())
                && !ServerInfo.Extensions.contains("!" + card->getPackage())) {
                names9 << card->objectName();
            }
        }
        if (!names9.isEmpty()) {
            Title *newtitle = new Title(this, translate("DelayedTrick"), IQSanComponentSkin::QSanSimpleTextFont::_m_fontBank.key(G_COMMON_LAYOUT.graphicsBoxTitleFont.m_fontFace), Config.TinyFont.pixelSize());
    ...
    if (!names5.isEmpty()) {
        foreach (const QString &cardname, names5) {
            CardButton *button = new CardButton(this, cardname, scale);
            connect(button, &CardButton::clicked, this, &GuhuoBox::reply);
    ...
    if (!names6.isEmpty()) {
        foreach (const QString &cardname, names6) {
            CardButton *button = new CardButton(this, cardname, scale);
            connect(button, &CardButton::clicked, this, &GuhuoBox::reply);
    ...
    if (!names7.isEmpty()) {
        foreach (const QString &cardname, names7) {
            CardButton *button = new CardButton(this, cardname, scale);
            connect(button, &CardButton::clicked, this, &GuhuoBox::reply);
    ...
    if (!names8.isEmpty()) {
        foreach (const QString &cardname, names8) {
            CardButton *button = new CardButton(this, cardname, scale);
            connect(button, &CardButton::clicked, this, &GuhuoBox::reply);
