...
HatanoKokoro = sgs.General(extension, "HatanoKokoro", "game", 3, false)
HatanoKokoro:addCompanion("Koishi")
...
UshioNagisa = sgs.General(extension , "UshioNagisa", "real", 3, false, true)
...
function getMaxSkinId(general_name)
	local n = 0
	for i = 1, 998, 1 do
		if file_exists("hero-skin/"..general_name.."/"..i.."/full.png") then
			n = i
...
UseDefaultSkin = sgs.CreateTriggerSkill{
	...
	on_record = function(self, event, room, player, data)  
		...
		if player and player:isAlive() and (string.find(player:getActualGeneral1Name(), "sujiang") or player:property("head_skin_id"):toInt() > getMaxSkinId(player:getActualGeneral1Name())) then
			room:setPlayerProperty(player,"head_skin_id", sgs.QVariant(0))
		end
		if player and player:isAlive() and (string.find(player:getActualGeneral2Name(), "sujiang") or player:property("deputy_skin_id"):toInt() > getMaxSkinId(player:getActualGeneral2Name())) then
...
function getActualUsedGeneral(room)
	local used = {}
	local removed_list = {}
	local removed = room:getTag("removed_general"):toList()
	for _,s in sgs.qlist(removed) do
		table.insert(removed_list, s:toString())
	end
	for _,p in ipairs(room:getUsedGeneral()) do
	   if not table.contains(removed_list, p) then
	       table.insert(used, p)
	   end
	end
	return used
...
Linggan = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, event, room, player, data)
		...
		for _,sp in sgs.qlist(players) do
		  ...
		  if (player ~= sp and sp:getMark("linggan1_used")==0) or (sp:getMark("linggan2_used")==0 and sp == player and sp:inHeadSkills(self) == data:toBool() and not has and player:getPhase() == sgs.Player_NotActive) then
		     return self:objectName(), sp
		  end
		end
	end,
...
ShunshanCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, player, targets)
		...
		local id = room:askForCardChosen(player, target, "hej", "shunshan")
        local card = sgs.Sanguosha:getCard(id)
...
ChenglingCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room, source, targets)
        ...
        for _,p in ipairs(targets) do
            ...
            if sgs.Sanguosha:getGeneral(g1) and g1 ~= p:getActualGeneral1Name() and not table.contains(getActualUsedGeneral(room), g1) then
				p:showGeneral()
                room:transformHeadGeneralTo(p, g1)
			end
			if sgs.Sanguosha:getGeneral(g2) and g2 ~= p:getActualGeneral1Name() and not table.contains(getActualUsedGeneral(room), g2) then
				p:showGeneral(false)
...
moshiCard = sgs.CreateSkillCard{
	...
	on_use = function(self, room,player,targets)
        local n = player:getMark("Nekojiyi_suit") 
		local choice=room:askForChoice(player,"moshi","moshi_top+moshi_bottom")
		if choice =="moshi_top" then
			for _,tar in ipairs(targets) do
				if not tar:isNude() then
				    local card_id = room:askForCardChosen(player, tar, "he", "moshi")
					local card = sgs.Sanguosha:getCard(card_id)
					local reason = sgs.CardMoveReason(0x09, player:objectName(), "moshi", "")
		...
		if n > 3 then 
		  local tw = room:askForPlayersChosen(player, room:getAlivePlayers(), "moshi", 0, 1, "moshi_tar")
...
moshiglobal = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, event, room, player, data, sp)
		if event == sgs.DamageInflicted then
			local damage = data:toDamage()
			if player:getMark("##moshi_replace")> 0 then
				return self:objectName()
			end
		end
		return ""
	...
	on_effect = function(self, event, room, player, data)
        local damage = data:toDamage()
		room:setPlayerMark(player, "##moshi_replace", 0)
		damage.damage = damage.damage - 1
...
Nekojiyi = sgs.CreateTriggerSkill{
	...
	on_record = function(self, event, room, player, data)
	    if event == sgs.EventPhaseEnd then
			if player:getPhase() == sgs.Player_Finish then
			    for _,p in sgs.qlist(room:getAlivePlayers()) do
				   room:setPlayerMark(p, "Nekojiyi_suit", 0)
			    end 
			end
		end
		--[[if event == sgs.CardUsed then
	...
	on_effect = function(self, event, room, player, data)
	    if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_RoundStart then
           room:setPlayerMark(player, "Nekojiyi_suit", 2)
		end   
	end
}
	...
	if event == sgs.Death then
		...
		local cards = splayer:getCards("he")
			for _,card in sgs.qlist(cards) do
				...
				local choice=room:askForChoice(splayer,self:objectName(),"yizhigame1+yizhigame2+yizhigame3")
					local list = room:getAlivePlayers()
					if choice == "yizhigame1" then
						
					player:gainMark("@yizhigame1", 1)	
					end
					if choice == "yizhigame2" then
						
					player:gainMark("@yizhigame2", 1)	
					end
					
					if choice == "yizhigame3" then
	...
	guilingCard = sgs.CreateSkillCard{
		...
		filter = function(self, targets, to_select)
			if #targets < sgs.Self:getMark("#SakiMark") and #targets < 3 then
				return to_select:objectName() ~= sgs.Self:objectName() and add_different_kingdoms(to_select, targets)
			end
			return false
		end,
		feasible = function(self, targets)
...
Baolie= sgs.CreateTriggerSkill{
	...
	Yinchang= sgs.CreateTriggerSkill{
		...
		can_trigger = function(self, event, room, player, data)
			...
			if event == sgs.DrawNCards and player:isAlive() and player:hasSkill(self:objectName()) and player:getPile("yinchang"):length()>2 then
				return self:objectName()
			end
			return ""
		end,
		on_cost = function(self, event, room, player, data)
			...
			if event == sgs.DrawNCards and (player:hasShownSkill(self:objectName()) or player:askForSkillInvoke(self, data)) then
				return true
			end
			return false
		end,
	...
	Yinchangmax = sgs.CreateMaxCardsSkill{
		name = "#yinchangmax",
		extra_func = function(self, player)
			if player:hasShownSkill("yinchang") and player:getPile("yinchang"):length()>2 then
			   return 1
			end
			return 0
		end
	...
	xinglingShown = sgs.CreateTriggerSkill{
		name = "#xinglingShown" ,
		events = {sgs.GeneralShown, sgs.CardUsed, sgs.EventPhaseChanging, sgs.TrickCardCanceling},
		on_record = function(self, event, room, player, data)
			if event == sgs.EventPhaseChanging then
				local change = data:toPhaseChange()
				if change.to == sgs.Player_NotActive then
		...
		can_trigger = function(self, event, room, player, data)
			...
			if event == sgs.CardUsed and player:isAlive() and player:hasSkill("xingling") and data:toCardUse().card:isKindOf("Eireishoukan") and not string.find(player:getActualGeneral2Name(), "sujiang") then
				return self:objectName()
			end
			if event == sgs.TrickCardCanceling then
				local effect = data:toCardEffect()
				if effect.card:getSkillName() == "xingling" then return self:objectName() end
			end
			return ""
		end,
		on_cost=function(self,event,room,player,data)
...
Aoshi = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, event, room, player, data)
		...
		--[[if event == sgs.PindianVerifying and player:hasSkill(self:objectName()) and player:isAlive() then
		   for _,c in sgs.qlist(player:getCards("e")) do
		       if c:isKindOf("EquipCard") then return self:objectName() end----c:isKindOf("Armor") or c:isKindOf("Horse")
		   end
		end]]
		if event == sgs.Pindian then
           local pd = data:toPindian()
		   if pd.reason ~= "aoshi" then return end
		    local n1 = pd.from_number
			local n2 = pd.to_number
			local card1 = pd.from_card
            local card2 = pd.to_card
			local success = pd:isSuccess()
			if success then
				local da = sgs.DamageStruct()
				target = pd.to
				player = pd.from
	...
	on_cost = function(self, event, room, player, data, sp)
			...
			if target then
				...
				if not ids:isEmpty() then
					...
					if card_id then
						...
						local pd = player:pindianSelect(target, "aoshi", sgs.Sanguosha:getCard(card_id))
						local da = sgs.QVariant()
						da:setValue(pd)
		...
		--[[else
		    if player:hasShownSkill(self:objectName()) or player:askForSkillInvoke(self, data) then return true end]]
	    end
	end,
	on_effect = function(self, event, room, player, data, sp)
        if event == sgs.EventPhaseStart then
        ...
        --[[if event == sgs.PindianVerifying then
			...
			local n = 0
			for _,c in sgs.qlist(player:getCards("e")) do
			    if c:isKindOf("EquipCard") then n = n+1 end---c:isKindOf("Armor") or c:isKindOf("Horse")
		    end
			local log = sgs.LogMessage()
			log.type = "$AoshiEffect"
			log.from = player
...
Xinshang = sgs.CreateTriggerSkill{
	name = "xinshang",
    events = {sgs.HpLost, sgs.PreHpLost, sgs.Dying, sgs.EventPhaseChanging},
	on_record = function(self, event, room, player, data)
        if event == sgs.PreHpLost and player:isAlive() and player:hasSkill(self:objectName()) then
			local n = data:toInt()
			if player:getHp()>n then
				room:setPlayerFlag(player, "xinshang_live")
			else
				room:setPlayerFlag(player, "xinshang_dying")
			end
		end 
		if event == sgs.EventPhaseChanging then
			local phase = data:toPhaseChange().to
			if phase == sgs.Player_NotActive and player:getMark("#aoshi_null")>0 then
				room:setPlayerMark(player, "#aoshi_null", player:getMark("#aoshi_null")-1)
			end
		end
	end,
	can_trigger = function(self, event, room, player, data)
		if event == sgs.HpLost and player:isAlive() and player:hasSkill(self:objectName()) and player:hasFlag("xinshang_live") then
			return self:objectName()
		end
		if event == sgs.Dying and player:isAlive() and player:hasSkill(self:objectName()) and player:hasFlag("xinshang_dying") then
			return self:objectName()
		end
		return ""
	end,
	on_cost = function(self, event, room, player, data, sp)
		...
		if event == sgs.Dying and (player:hasShownSkill(self:objectName()) or player:askForSkillInvoke(self,data)) then
			room:setPlayerFlag(player, "-xinshang_dying")
			room:broadcastSkillInvoke(self:objectName(), player)
			return true
		end
	...
	on_effect = function(self, event, room, player, data, sp)
		if event == sgs.HpLost then
		   local choice = room:askForChoice(player, self:objectName(), "xinshang_draw+xinshang_use")
           if choice == "xinshang_draw" then player:drawCards(1) end
		   if choice == "xinshang_use" then
			...
			if not player:isKongcheng() then room:askForUseCard(player, table.concat(pattern, ",").."|.|.|hand", "@xinshang_use", -1, sgs.Card_MethodUse, false) end
	       end
		end
		if event == sgs.Dying then
			---local x = player:getMaxHp()+1
		    if player:getHandcardNum()<5 then player:drawCards(5-player:getHandcardNum()) end
			local a = 1
...
xuemai = sgs.CreateTriggerSkill{
	name = "xuemai",
	frequency = sgs.Skill_Compulsory,
	events = {sgs.GeneralShown, sgs.CardUsed},
	can_trigger = function(self, event, room, player, data)
        if event == sgs.GeneralShown then
        ...
        if event == sgs.CardUsed then
			local use = data:toCardUse()
			if player and player:isAlive() and player:hasSkill(self:objectName()) and (sgs.Sanguosha:getCurrentCardUseReason() == 0x01 or sgs.Sanguosha:getCurrentCardUseReason() == 0x12) and (use.card:isKindOf("Peach") or use.card:isKindOf("Analeptic") or use.card:getSubtype() == "food_card") then
				return self:objectName()
			end
		end
		return ""
	...
	on_effect = function(self, event, room, player, data)
	    if event == sgs.GeneralShown then
		    if room:getDrawPile():length() > 0 then
                ...
                for _,id in sgs.qlist(room:getDrawPile()) do
                    local card = sgs.Sanguosha:getCard(id)
                    if not ((card:isKindOf("Peach") or card:isKindOf("Analeptic") or card:getSubtype() == "food_card")) then
                        continue
                    end
                    list:append(id)
                end
        ...
        if event == sgs.CardUsed then
		    if room:getDrawPile():length() > 0 then
                ...
                if listA:length() > 0 then
                    local id = listA:at(math.random(0, listA:length()-1))
                    local card = sgs.Sanguosha:getCard(id)
                    room:obtainCard(player, card)
                end
                if listB:length() > 0 then
                    local id = listB:at(math.random(0, listB:length()-1))
                    local card = sgs.Sanguosha:getCard(id)
                    room:obtainCard(player, card)
                end
				room:setPlayerFlag(player, "xuemai_used")
            end
		end		  
	end	   
}

Xuemaimax = sgs.CreateMaxCardsSkill{
	name = "#xuemaimax",
	extra_func = function(self, player)
		if player:hasFlag("xuemai_used") then
           return 1
		end
		return 0
	end
...
Xuemaimod = sgs.CreateTargetModSkill{
	name = "#xuemaimod",
	residue_func = function(self,player,card)
	  if player:hasFlag("xuemai_used") then return 1 end
	end 
}
...
mianlingCard = sgs.CreateSkillCard{
	name = "mianlingCard",
	target_fixed = true,
...
mianlingShown = sgs.CreateTriggerSkill{
	...
	on_record = function(self, event, room, player, data)
		...
		if event == sgs.CardsMoveOneTime then
			local move = data:toMoveOneTime()
			if (player:isDead() or not player:hasShownSkill("mianling")) then return "" end
		...
		if event == sgs.DrawNCards then
			...
			for _,sp in sgs.qlist(players) do
					...
					if list:length()>0 then
						...
						for _,g in sgs.qlist(room:getDrawPile()) do  if cards:objectName() ~= sgs.Sanguosha:getCard(g):objectName() then  lists:append(g)  end	end
						if lists:length() > 0 then	local ids = lists:at(math.random(0, lists:length()-1))  local card = sgs.Sanguosha:getCard(ids)  room:obtainCard(player, card)	end
					end
					count = count - 10
					data:setValue(count)
				end	
		...
		if event == sgs.CardUsed then
			...
			for _,sp in sgs.qlist(players) do
			        ...
			        if choice == "thunder_slash" then
					    local log = sgs.LogMessage()  log.type = "$UseCard_thunder"  log.from = player  log.arg = self:objectName()  log.card_str = use.card:toString()   room:sendLog(log)
						...
						use.card=newslash
						data:setValue(use)
					elseif choice == "fire_slash" then
					    local log = sgs.LogMessage()  log.type = "$UseCard_fire"  log.from = player  log.arg = self:objectName()  log.card_str = use.card:toString()  room:sendLog(log)
					    local newslash=sgs.Sanguosha:cloneCard("fire_slash",use.card:getSuit(),use.card:getNumber())
						newslash:addSubcard(use.card:getId())
						use.card=newslash
						data:setValue(use)
					elseif choice == "ice_slash" then 
					    local log = sgs.LogMessage()  log.type = "$UseCard_ice"  log.from = player  log.arg = self:objectName()  log.card_str = use.card:toString()  room:sendLog(log)
					    local newslash=sgs.Sanguosha:cloneCard("ice_slash",use.card:getSuit(),use.card:getNumber())
						newslash:addSubcard(use.card:getId())
						use.card=newslash
						data:setValue(use)
					end
	...
	can_trigger = function(self, event, room, player, data)
		if event == sgs.GeneralShown and player and player:isAlive() and player:inHeadSkills(self) == data:toBool()and player:hasSkill("mianling")and player:getMark("&mianling_Shown") < 1then return self:objectName() end
		return ""
    end,
	on_cost=function(self,event,room,player,data)
        if player:hasShownSkill("mianling") or player:askForSkillInvoke(self, data) then
		    room:setPlayerMark(player, "&mianling_Shown", 1)
			room:broadcastSkillInvoke("mianling", player)
            return true
		end
		return false
	end,
...
mianlingnu = sgs.CreateTargetModSkill{
	name = "#mianlingnu",
	pattern = "Slash",
	residue_func = function(self, player, card)
		if player:getMark("mianling_nu_Slash") > 0 then
			return player:getMark("mianling_nu_Slash")
		end	return 0
	end,
}

mianlingai = sgs.CreateMaxCardsSkill{
	name = "mianlingai",
	extra_func = function(self, player)
		if player:hasFlag("mianling_ai_extra") then
			return - 1
		end return 0
	end
}

ranxv = sgs.CreateTriggerSkill{
	name = "ranxv",
	events = {sgs.EventPhaseStart},
	can_trigger = function(self, event, room, player, data)
       if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Play then
	...
	on_effect = function(self, event, room, player, data, sp)
	    if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Play then
			...
			if card then
				room:moveCardTo(card, player, nil, sgs.Player_DiscardPile, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, player:objectName(), self:objectName(), ""))
			...
			if --[[not list:isEmpty()]]true then --没有花色也要询问避免暴露手牌
				if sp:askForSkillInvoke("ranxv_recast") and not list:isEmpty() then	
					room:fillAG(list, sp)
					local dat = sgs.QVariant()
					dat:setValue(sp)
					room:setPlayerProperty(sp, "ranxv_target", dat)
					--local id = room:askForAG(sp, list, false, self:objectName()) 可选择不选避免只有1张可选时自动选择从而暴露手牌结构
					local id = room:askForAG(sp, list, true, self:objectName())
					room:setPlayerProperty(sp, "ranxv_target", sgs.QVariant())	
					room:clearAG(sp)
					if id < 0 then return false end --没有选择则结束
					local cardsp = sgs.Sanguosha:getCard(id)
					if cardsp then
						room:moveCardTo(cardsp, sp, nil, sgs.Player_DiscardPile, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, sp:objectName(), self:objectName(), ""))
						room:broadcastSkillInvoke("@recast")
						local log = sgs.LogMessage()
						log.type = "$RecastCard"
						log.from = sp
						log.card_str = cardsp:toString()
						room:sendLog(log)
						sp:drawCards(1, "recast")
					end
					if sp:isAlive() then room:obtainCard(sp, card) end
				end	
			end	
		end  	
	end,
...
zhilians = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, event, room, player, data)
		local damage = data:toDamage()
		if player:isAlive() and player:hasSkill("zhilians") then
			for _,p in sgs.qlist(room:getOtherPlayers(player)) do 
			    if p:getMark("@lian")>0 then
					return self:objectName()
				end
			end
		end
	...
	on_cost = function(self, event, room, player, data, ask_who)
		if player:askForSkillInvoke(self, data) then room:broadcastSkillInvoke(self:objectName(), player) return true end
		return false
	end,
	on_effect = function(self, event, room, player, data, ask_who)
		local damage = data:toDamage()
...
zhiliansShown = sgs.CreateTriggerSkill{
	...
	on_effect=function(self, event, room, player, data)
	    if event == sgs.GeneralShown then
		    local targets = room:askForPlayersChosen(player, room:getOtherPlayers(player), self:objectName(), 0, 1, "@zhilians")
			for _,p in sgs.qlist(targets) do
				room:broadcastSkillInvoke("zhilians", 1)
				room:doLightbox("zhilians$", 1000)
			    room:setPlayerMark(p, "@lian", 1)
			end
		end
...
yixinsS = sgs.CreateViewAsSkill{
	...
	view_filter = function(self, selected, to_select)
		...
		if sgs.Self:getLostHp() > 0 and #selected >= sgs.Self:getLostHp() then return false end
		if sgs.Self:getLostHp() == 0 and  #selected >= 1 then return false end
		return not sgs.Self:isJilei(to_select)
	end,
	view_as = function(self, cards)
		if #cards == 0 then return nil end
		local vs = yixinsCard:clone()
		vs:setShowSkill(self:objectName())
		vs:setSkillName(self:objectName())
		for i = 1, #cards, 1 do
	...
	enabled_at_response = function(self, player, pattern)
		return pattern == "@@yixins"
	end,
}

yixinsCard = sgs.CreateSkillCard{
	name = "yixinsCard",
	will_throw = false,
	filter = function(self, targets, to_select) 
		if #targets <1 then
	...
	on_use = function(self, room, player, targets)
	    local card = self:getSubcards():length()
	    --[[room:moveCardTo(self, player, nil, sgs.Player_DiscardPile, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST, player:objectName(), "yixins", ""))
		room:broadcastSkillInvoke("@recast")
		local log = sgs.LogMessage()
		...
		--local tar = room:askForPlayersChosen(player, room:getAlivePlayers(), "yixins", 0, 1, "@yixins_tar")
		for _,p in ipairs(targets) do
		    local card = sgs.Sanguosha:cloneCard("slash")
			card:setSkillName("yixins")
			for _,i in sgs.qlist(self:getSubcards()) do  
			   card:addSubcard(i) 
			end	
			p:addToPile("yixin", card, false)
...
yixins = sgs.CreateTriggerSkill{
	name = "yixins",
	view_as_skill = yixinsS,
	can_preshow = true,
	events = {sgs.HpLost, sgs.Damaged},---, sgs.EventPhaseStart
	--[[on_record = function(self, event, room, player, data)
	...
	on_effect = function(self, event, room, player, data)
		player:drawCards(math.max(1, player:getLostHp()))
		room:askForUseCard(player, "@@yixins", "@yixins")	
	end,
}  
...
yixinsMaxCards = sgs.CreateMaxCardsSkill{
	name = "yixinsMaxCards",
	extra_func = function(self, player)
		if player:hasFlag("yixins_max") then return 1 end
		return 0
	end
}
...
hezous = sgs.CreateTriggerSkill{
	...
	on_record = function(self, event, room, player, data)
		if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Draw then
			for _,sp in sgs.qlist(room:getAlivePlayers()) do
				if sp:getPile("yixin"):length()> 0 and room:getCurrent():objectName() == sp:objectName() then
					if  sp:getMark("hezous_target") == 0 then
						local dummy = sgs.DummyCard(sp:getPile("yixin"))
						room:obtainCard(sp, dummy)
						room:setPlayerFlag(sp, "yixins_max")
						room:broadcastSkillInvoke("yixins",4)
					elseif  sp:getMark("hezous_target") > 0 then
					    local YP = sp:getPile("yixin")
						local left = YP
					...
					room:setPlayerMark(sp, "hezous_target", 0)
				end
			end
		end
		if event == sgs.CardUsed then
	...
	can_trigger = function(self, event, room, player, data)
		if event == sgs.Death then
			...
			if player:hasSkill(self:objectName())and player == death.who and not player:isNude() then
                for _,spl in sgs.qlist(room:getOtherPlayers(player)) do
	...
	on_effect = function(self, event, room, player, data)
		...
		room:setPlayerMark(dy, "hezous_target", 1)
		--[[local dummy = sgs.DummyCard()----所有牌置于目标“遗信”牌堆
		for _,id in sgs.qlist(player:getCards("he")) do dummy:addSubcard(id) end
		dy:addToPile("yixin", dummy)
		dummy:deleteLater()]]
	end,
}
...
Dingxin = sgs.CreateTriggerSkill{
    name = "dingxin",
	frequency = sgs.Skill_Compulsory,
    events = {sgs.EventPhaseStart, sgs.Dying, sgs.EventPhaseEnd},
	on_record = function(self, event, room, player, data)
		if event == sgs.EventPhaseEnd then
			if player:hasFlag("dingxin_max") and player:getPhase() == sgs.Player_Discard then
                room:setPlayerFlag(player, "-dingxin_max")
				local _type = ".|heart|.|hand"
    ...
    can_trigger = function(self, event, room, player, data)
        if event == sgs.EventPhaseStart then
			...
			if player and player:isAlive() and player:hasSkill(self:objectName()) and player:getPhase() == sgs.Player_Discard then
				return self:objectName()
			end
        end
		if event == sgs.Dying then
           local dying = data:toDying()
		   if player and player:isAlive() and player:hasSkill(self:objectName()) and dying.who:objectName() == player:objectName() then
			   return self:objectName()
		   end
		end
        return ""
    ...
    on_cost = function(self, event, room, player, data, sp)
		...
		if event == sgs.Dying then
			if player:askForSkillInvoke(self, data) then
				...
				for _,p in sgs.qlist(players) do
					if not p:isFriendWith(player) then continue end
					...
					for _,c in sgs.qlist(cards) do
						list:append(c:getEffectiveId())
						if c:getSuitString() ~= "heart" then dis:append(c:getEffectiveId()) end
                    end
				...
				if n == 12 then
					local move = sgs.CardsMoveStruct()
					move.card_ids = discard
				    move.reason = sgs.CardMoveReason(0x01,"","dingxin","")
				    move.to_place = sgs.Player_DiscardPile
				    room:moveCardsAtomic(move, true)
                    return true
				end
	...
	on_effect = function(self, event, room, player, data, sp)
		...
		if event == sgs.Dying then
			local head, deputy
			if player:getActualGeneral1Name() == "Ushio" then head = true end
			if player:getActualGeneral2Name() == "Ushio" then deputy = true end
			if head or deputy then
				room:broadcastSkillInvoke(self:objectName(), 19, player)
                room:doLightbox("dingxin$", 2000)
			end
			if head then
				player:showGeneral(true)
				room:transformHeadGeneralTo(player, "UshioNagisa")
			end
			if deputy then
				player:showGeneral(false)
				room:transformDeputyGeneralTo(player, "UshioNagisa")
			end
			if (head or deputy) and not table.contains(room:getUsedGeneral(), "Ushio") then
				room:handleUsedGeneral("Ushio")
			end
...
Dingxinmax = sgs.CreateMaxCardsSkill{
	name = "#dingxinmax",
	extra_func = function(self, player)
		if player:hasShownSkill("dingxin") then
			local n = 0
			for _,c in sgs.qlist(player:getHandcards()) do
			   if c:getSuitString() == "heart" then n = n+1 end
		    end
			return n
		end
		return 0
...
Kazoku = sgs.CreateTriggerSkill{
	...
	on_record = function(self, event, room, player, data)
		if event == sgs.AskForRetrial then
			...
			local leader
			for _,p in sgs.qlist(room:getAlivePlayers()) do
				if p:isFriendWith(player) and p:hasShownSkill("kazoku") then leader = true end
			end
			if player == judge.who and judge.card:isRed() and judge.reason == "keyCard" and leader then
				judge.pattern = ".|red|."
				judge:updateResult()
				local msg = sgs.LogMessage()
				msg.from = player
				msg.type = "#KazokuEffect"
				msg.arg = "kazoku"
				room:sendLog(msg)
			end
		end
    ...
    on_cost = function(self, event, room, player, data, sp)
        if event == sgs.EventPhaseStart then
			...
			if player:askForSkillInvoke(self, data) then
				local dest = room:askForPlayerChosen(player, list, self:objectName(), "", true, true)
				if dest then
				   player:setProperty("kazoku_dest", sgs.QVariant(dest:objectName()))
				   room:broadcastSkillInvoke(self:objectName(), player)
				   return true
				end
			end	
	...
	on_effect = function(self, event, room, player, data, sp)
		if event == sgs.EventPhaseStart then
			local name = player:property("kazoku_dest"):toString()
			local dest = findPlayerByObjectName(name)
			if not dest or dest:isDead() then return end
			player:setProperty("kazoku_dest", sgs.QVariant())
			local card
			local cards = {}
			for _,i in sgs.qlist(room:getDrawPile()) do
				if sgs.Sanguosha:getCard(i):getSuitString() == "heart" then
...
sgs.insertRelatedSkills(extension, "yinchang", "#yinchangmax")
...
Sakunahime:addSkill(xuemai)
sgs.insertRelatedSkills(extension, "xuemai", "#xuemaimax", "#xuemaimod")
HatanoKokoro:addSkill(mianling)
HatanoKokoro:addSkill(mianlingShown)
sgs.insertRelatedSkills(extension, "mianling", "#mianlingShown")
HatanoKokoro:addSkill(ranxv)
...
sgs.insertRelatedSkills(extension, "dingxin", "#dingxinmax")
UshioNagisa:addSkill(Kazoku)
...
if not sgs.Sanguosha:getSkill("moshiglobal") then skills:append(moshiglobal) end
...
if not sgs.Sanguosha:getSkill("#mianlingnu") then skills:append(mianlingnu) end
if not sgs.Sanguosha:getSkill("mianlingai") then skills:append(mianlingai) end
if not sgs.Sanguosha:getSkill("#xuemaimax") then skills:append(Xuemaimax) end
if not sgs.Sanguosha:getSkill("#xuemaimod") then skills:append(Xuemaimod) end
if not sgs.Sanguosha:getSkill("#yinchangmax") then skills:append(Yinchangmax) end
if not sgs.Sanguosha:getSkill("#dingxinmax") then skills:append(Dingxinmax) end
...
sgs.LoadTranslationTable{
  ...
  [":yuanyu"] = "<font color=\"green\"><b>每轮限一次，</b></font>当你的牌因弃置进入弃牌堆时，你可以选择其中一张记为“缘遇”牌并与牌堆顶五张牌以任意顺序置于牌堆顶。一名其他角色使用对应实体牌为“缘遇”牌的牌时，清除此牌“缘遇”记录，然后你摸3张牌并可以交给其任意张牌。",
  ["@yuanyu"] = "交给该角色任意张牌", 
  ...
  ["cv:Sakuta"] = "石川界人",
  ["shuaiyan"] = "率言",
  [":shuaiyan"] = "<font color=\"green\"><b>回合内限一次，</b></font>当你使用牌指定一名其他角色为唯一目标后，你可以获得其区域内的一张牌，然后若其存活且与你势力相同，你摸一张牌，若其存活且其为女性，其摸一张牌。",
  ["shangxian"] = "伤现",
  ...
  ["mjianshi"] = "渐失",
  [":mjianshi"] = "出牌阶段限一次，你可以选择永久跳过一个判定/摸牌/出牌/弃牌阶段(下次进入该阶段时生效，需有明置的此技能才生效)，并选择场上一名其他玩家，选择一项： 1，其下个回合开始时获得一个该阶段。 2，其下次执行此阶段时跳过该阶段；然后你选择你或其摸x张牌(x为你本局游戏永久跳过阶段数)。锁定技，回合结束时，若你{手牌数}大于{你手牌上限+1}，你选择弃置2张手牌或失去1点体力。",
  ["@jiaozhi"] = "交织：选择角色",
  ["Judge"] = "判定阶段",
  ["Draw"] = "摸牌阶段",
  ["Play"] = "出牌阶段",
  ...
  ["cv:Zero"] = "花守由美里",
  ["zhufa"] = "著法「零之书」",
  [":zhufa"] = "<font color=\"green\"><b>每回合限一次，</b></font>当你使用一张普通锦囊牌时，你可以记录此牌名称并发动一次单体势力召唤，若有人响应，其获得此牌。出牌阶段限一次，你可以将一张手牌当作一张记录牌使用。",
  ["fashufengyin"] = "封印「法书封印」",
  [":fashufengyin"] = "一名其他角色使用一张指定目标的“著法”记录牌名的牌时，你可以弃置一张牌，令此牌无效。",
  ["liqi"] = "立契",
  [":liqi"] = "限定技，出牌阶段，你可以指定一名势力相同的其他角色；<font color=\"#C0C0C0\"><b>永久技，</b></font>该角色受到不为实体【杀】的伤害时，你代为承受该伤害；你/其的出牌阶段开始时，其/你可以选择自己一张牌交给你/其。",
  ["@liqi"] = "立契",
  ["@liqi_dest"] = "佣兵",
  ["liqieffect"] = "立契",
  ["%Zero"] = "“吾辈觉得能与你相遇真是太好了”",
  ...
  ["bishi"] = "避矢",
  [":bishi"] = "当你受到其他角色的伤害时，若其与你的距离大于1且此人物牌明置，你可以选择暗置该人物牌，防止此伤害。",
  ["cichuan"] = "刺穿「Gáe Bolg」",
  [":cichuan"] = "结束阶段结束时，你可以展示你手牌中一张♦牌并指定一名攻击范围内人物牌上没有“枪”的其他角色，你对其造成1点伤害，若该角色存活，你将此牌置于该角色人物牌上，称为“枪”。<font color=\"#C0C0C0\"><b>永久技，</b></font>当人物牌上存在“枪”的角色回复体力时，弃置人物牌上的第一张“枪”，取消之。",
  ["qiang"] = "枪",
  ["@cichuan"] = "刺穿「Gáe Bolg」",
...
[":yaoshi"] = "<font color=\"green\"><b>每回合限一次，</b></font> ①当一名角色进入濒死状态时，你可以弃置一张♥牌，令其回复1点体力。②一名角色出牌阶段开始时，你可以弃置一张手牌，若该角色出牌阶段使用的第一张牌与你弃置的牌花色不同，其受到你的1点伤害；若花色相同，你摸一张牌。",
["@yaoshi"] = "药师：弃置一张手牌",
["@yaoshi2"] = "药师：弃置一张红桃牌",
["shenxue"] = "神血「解放者之血」",
["Fl_shenxue$"] = "image=image/animate/Fl_shenxue.png",
["$shenxue"] = "爆炸音效",
[":shenxue"] = "当其他同势力角色阵亡或你进入濒死状态时，你可以摸一张牌并翻面，若因此处于叠置状态，令所有其他角色选择一项执行：1、弃置两张手牌。2、失去1点体力。",
--["@shenxue"] = "神血：选择至多两名势力不同的角色",
["@shenxue"] = "弃置两张手牌否则失去1点体力",
["@shenxue-discard"] = "神血：弃置两张手牌否则失去1点体力",
["shenxue_recover"] = "回复1点体力",
...
["cv:PYuuki"] = "阿部敦",
["pquanneng"] = "权能「公主骑士之力」",
[":pquanneng"] = "<font color=\"green\"><b>每回合限一次，</b></font>当一名角色造成伤害时，若其存在明置女性人物牌，你可以摸一张牌，令该角色选择一项:1.观看你的手牌，并获得其中一张；2.获得一枚“蓄力”标记（场上存在此标记时你的手牌上限+1）；3.弃置人物牌上的一个“蓄力”标记，摸一张牌并回复1点体力。",
["@xuli"] = "蓄力",
["@haogandu"] = "好感度",
...
["plianjie"] = "连结「与你的羁绊」",
[":plianjie"] = "主将技，你计算体力上限减少1个单独的阴阳鱼。锁定技，当你发动一次“权能”后或者造成一次伤害时，获得一个“好感度”标记(上限为3)；你的准备阶段开始时，若你“好感度”标记为3，你可以弃置所有“好感度”标记，并从一个随机常规势力的三个随机未登场女性人物牌中选择一个替换你的副将。",--其他角色准备阶段开始时，若你有以此法记录的副将且其不在场上，你可以清除记录并替换为此副将，以此法替换副将时，不重置其限定技。",
...
["cv:Estelle"] = "神田朱未",
["fenglun"] = "凤轮",
[":fenglun"] = "出牌阶段限一次，你可弃置一张牌并选择一名角色，根据牌的类型令其选择是否执行以下效果：基本牌，弃置一张【杀】；装备牌，弃置两张手牌；锦囊牌，你摸一张牌并视为对其使用一张【决斗】。若为否，其受到你的1点伤害。",
...
["linggan"] = "灵感",
[":linggan"] = "<font color=\"green\"><b>每回合各限一次，</b></font>一名其他角色明置人物牌时，你可以观看牌堆底三张牌，并可以用一张手牌替换其中一张；你于回合外明置此人物牌时，若你判定区没有【节能主义】，你可以摸两张牌，然后将牌堆顶一张牌当作【节能主义】放于你判定区。",
["jieneng"] = "节能",
[":jieneng"] = "锁定技，弃牌阶段/判定阶段开始时，若你判定区有“节能主义”，此回合你手牌上限+3（不叠加）。",
["tuili"] = "推理",
[":tuili"] = "当一名角色使用一张基本或锦囊牌时，若你手牌或牌堆底三张牌中有与其同名牌，你可以观看牌堆底3张牌，并可以弃置手牌或观看牌中的一张同名牌，令此技能此回合失效，然后你令1～2名角色各摸一张牌，<font color=\"green\"><b>每轮限一次，</b></font>{若你弃置的为手牌，你可暗置此人物牌}。",
["@linggan"] = "选择一张手牌交换",
["@tuili"] = "弃置一张同名牌",
...
["sorazhi"] = "空智",
[":sorazhi"] = "主将技，<font color=\"green\"><b>每回合限两次，</b></font>当你手牌数发生变化时，如果场上存在其他与你手牌数相等的角色，你可以选择其中一名角色，令其摸一张牌或者弃一张牌。",
...
["shiroshi"] = "白识",
[":shiroshi"] = "副将技，当你体力值发生变化时，你可以预知X（X为场上与你体力相等的角色数），然后你可以令与你体力值相等的一名角色摸一张牌，然后你可以视为对其攻击范围的一名角色使用一张“心灵读取”",
...
["longhua"] = "龙化",
["$longhua"] = "吼~~（龙吼声）",
[":longhua"] = "主将技，限定技，准备阶段开始时，若你“历”数量不小于体力上限且你体力不大于1，你可以增加1点体力上限并回复1点体力，然后你将操铠X改为：弃置你判定区一张牌，你的【杀】造成的非传导伤害+1，额定目标+1。锁定技，当你回合结束时，若你发动此限定技且“历”标记小于体力上限，移除此人物牌。",
["lizhan_mark"] = "历",
["#lizhan_mark"] = "历",
["@longhua"] = "龙化",
["%Tatsumi"] = "“恶鬼缠身！”",
...
["$sshiji2"] = "时间啊......停止吧！",
[":sshiji"] = "<font color=\"green\"><b>每回合限一次，</b></font>当你因“幻身”弃置牌时，若弃置牌颜色相同，且弃置牌不小于3或你没手牌，你可以回复1点体力，若至少有三张花色相同，你可以移动一名角色判定区所有牌，若有三种类别，你可以令当前回合角色叠置，若至少有三张同名，你可以从弃牌堆随机获得三张不同名的牌，且至你下个回合结束时你使用杀的额定次数+1。",
...
["bingshu"] = "冰术",
[":bingshu"] = "你可以将一张黑色基本牌当作冰【杀】使用或打出，以此法使用无距离限制，额定目标数+1，每回合首次使用或打出时摸一张牌。",
["lingshi"] = "灵使",
...
["$chengling2"] = "柠檬召唤！",
[":chengling"] = "限定技，出牌阶段，你可以选择至多两名其他无暗置人物牌的角色，其依次执行：若其主/副人物与其游戏开始时不同且其游戏开始时的主/&副人物不在场(指已经移除的人物牌，或未加入的人物牌)，主/&副人物变更为游戏开始时对应人物；其对应位置获得当前主副人物牌上没有的技能，并重置当前主副人物拥有的限定技，然后其体力、体力上限调整至其当前主副人物组合的满状态。",
["%Chiyuri"] = "“柠檬召唤！”",
...
["cv:Rean"] = "内山昂辉",
["huanqi"] = "唤骑",
[":huanqi"] = "主将技，你计算体力上限减少1个单独的阴阳鱼。<font color=\"green\"><b>每回合限一次，</b></font>当你需要使用或打出一张【杀】时，你可以变更副将视为你使用或打出了一张【杀】。",
["rwuren"] = "无仞",
...
["cv:Joshua"] = "斋贺弥月",
["huanxi"] = "幻袭",
[":huanxi"] = "出牌阶段限一次，你执行下列一项，并选择一名其他角色，令其选择执行其他一项：1.弃置一张基本牌；2.弃置一张装备牌；3.失去1点体力。无法执行的角色本回合获得效果：本回合无法使用或打出手牌，不能回复体力，且你到其距离为1。",
["jjueying"] = "绝影",
[":jjueying"] = "锁定技，你的体力值大于体力上限一半时，你的回合内可以额外使用一张♠【杀】；你的体力值不大于体力上限一半时，你的“幻袭”可以再指定一名其他角色执行剩余一项。",
["@huanxi-basic"] = "弃置一张基本牌否则视为选择“失去1点体力”",
["@huanxi-equip"] = "弃置一张装备牌否则视为选择“失去1点体力”",
...
["eryu"] = "二羽",
	...
	["$eryu2"] = "翔鹤姐姐还好吗？",
	["$eryu3"] = "感觉很好呀♪",
	[":eryu"] = "阵法技，<font color=\"green\"><b>每回合限三次，</b></font>当一张基本牌或普通锦囊牌指定目标后，若你为围攻角色且目标包含被围攻角色，且使用者为围攻角色，（你令）使用者可以将此牌交给另一名围攻角色。",
	["youdiz"] = "诱敌",
	...
	["rinjiuyuan"] = "救援",
	[":rinjiuyuan"] = "主将技，一名角色进入濒死时，若其判定区没有【键】，你可以将牌堆顶一张牌当作【键】置于其判定区；判定区有【键】的角色受到致命伤害时，你可以弃置其判定区的一张【键】。",
    ...
    ["nizhuan"] = "逆转",
	[":nizhuan"] = "主将技，你计算体力上限减少1个单独的阴阳鱼。<font color=\"green\"><b>每回合限一次，</b></font>你可以将一张“证据”当做“异议”使用或打出，若你以此法使用牌的花色与被响应的【杀】牌颜色相同，此异议结算后目标视为对使用者使用一张名称相同的【杀】。",
	["yanshen"] = "延审",
...
["moshi"] = "魔使",
[":moshi"] = "出牌阶段限一次，可以根据你本回合已使用过的花色数执行：将至多等量角色各一张牌置于牌堆顶/弃置，若花色数达到4，令一名角色下次受到的伤害-1（触发标识为“防护罩”）。",
...
["nekojiyi"] = "汲忆",
[":nekojiyi"] = "回合开始时，若你手牌数为全场最少，你可以令本回合“魔使”已使用牌花色记录+2。",
["$nekojiyi"] = "青梅竹马？",
...
["Oumashu"] = "樱满集",
  ...
  ["wangguo"] = "王国",
  [":wangguo"] = "主将技，你计算体力上限减少1个单独的阴阳鱼。你使用虚空时可以额外指定一名其他势力相同的角色为目标；你使用虚空指定势力相同的角色为目标时，可以失去1点体力，执行下列一项：1，令其回复1点体力；2，其摸2张牌，；3，弃置其判定区一张牌。",
  ["void_draw"] = "令目标摸两张牌",
  ["void_recover"] = "令目标回复1点体力",
...
["ranxin"] = "燃心",
[":ranxin"] = "<font color=\"green\"><b>每回合限三次，</b></font>你使用/以使用方式打出一张红色牌时，若你“魔”小于3，你可以将牌堆顶一张牌作为“魔”置于人物牌上。锁定技，你有“魔”时，使用杀额定次数+1，你的杀造成伤害时弃置一张“魔”并摸一张牌。",
["$ranxin1"] = "因为...我发现了，其实我..早就已经被幸福包围了",
["$ranxin2"] = "现在我圆了梦，也有了美妙的回忆，已经没有什么遗憾了吧",
["chiyi"] = "斥忆",
[":chiyi"] = "锁定技，结束阶段结束时，若你有“魔”，你弃置所有“魔”，若弃置“魔”数不大于2，你失去1点体力，摸2张牌；若弃置“魔”数不小于3，你减1点体力上限，摸3张牌，并弃置一张手牌。",
["ranxin_magic"] = "魔",
...
["$zhanyuan2"] = "你好~这里是快捷便宜又放心的外派神明夜斗",
	...
	["sibie"] = "死别",
	[":sibie"] = "出牌阶段，你可以对一名体力值大于你的角色造成1点伤害，然后失去1点体力，然后，此回合该角色是此技能唯一合法目标。如果你因此死亡，该角色弃置装备区内所有牌。",
    ...
    ["guiling"] = "归零",
    [":guiling"] = "回合结束时，你须弃置所有“咲”。出牌阶段限一次，你可以选择1~3名其他势力不同的角色并弃置等量“咲”，这些角色依次选择一项：1.交给你一张装备牌；2.你回复1点体力；3.你对其造成1点伤害。",
    ["guilingglobal"] = "归零",
...
["@baolie"] = "爆裂",
["~baolie"] = "对任意名角色依次分配X点火焰伤害，每名角色分配数不超过2，重复点击角色令分配+1",
["yinchang"] = "吟唱",
...
["%Lucy"] = "“我想留在公会里，我最喜欢妖精的尾巴了!”",
  ...
  ["xingling"] = "星灵",
  [":xingling"] = "明置此人物牌时，你可以获得场上所有♦牌。<font color=\"green\"><b>出牌阶段限一次，</b></font>你可以将一张♦牌当作不可被金色宣言响应的【英灵召唤】使用。当你使用【英灵召唤】时，可以将副人物牌放在人物牌上，称为“星灵放置”。<font color=\"#C0C0C0\"><b>永久技，</b></font>回合结束时，若你有“星灵放置”，选择一项：1，弃置一张“星灵放置”。2，将副将变更为一张“星灵放置”，不以此法更新限定技。",
  ["xingling$"] = "image=image/animate/xingling.png",
  ["#xinglingShown"] = "星灵",
  ["$xingling"] = "语音",
  ["lingqi"] = "灵契",
  [":lingqi"] = "当你杀死一名角色时，你可以随机获得摸牌/弃牌堆的一张♦牌。",
  ["$lingqi"] = "语音",
...
["%ShionNezumi"] = "“一定会再相见！”",
["kangshi"] = "抗市",
[":kangshi"] = "当场上有除你之外的唯一大势力时：①出牌阶段限一次，你可以受到1点无来源伤害，依次获得场上的大势力角色的一张牌，然后此回合你到这些角色距离视为1；②你可以令你摸牌阶段额定摸牌数+1。",
["qingban"] = "情绊",
[":qingban"] = "<font color=\"green\"><b>每名角色限一次，</b></font>当一名角色进入濒死状态时，若你体力上限大于1，你可以减1点体力上限，令其回复1点体力。",
	...
	[":xinkong"] = "出牌阶段开始时，可以指定一名其他角色A，选择一项：1.重铸一张牌并令角色A展示手牌；2.指定A以外的一名角色为B，你观看A手牌并获得其中1张，A选择对B使用一张杀或失去1点体力，本回合你手牌上限+2并结束出牌阶段。",
	["xinkong$"] = "image=image/animate/xinkong.png",
	["@xinkong1"] = "「强制自白」：重铸一张牌并令角色A展示所有手牌",
	["@xinkong2"] = "「行为控制」：观看并获得A一张手牌，令A对B使用一张杀否则其失去1点体力，本回合你手牌上限+2并结束出牌阶段",
	["@xinkonga"] = "选择一名其他角色A",
	...
	["gongqing"]="共情",
	[":gongqing"]="<font color=\"green\"><b>每回合限一次，</b></font>当你获得一名其他角色的牌时/一名其他角色获得你的牌时，你可以弃置一张手牌，令其回复1点体力或手牌补至体力上限（最多摸5张）。",
	["vshoujivs"]="手记",
	["~vshouji"]="选择牌交给目标",
	...
	["$vshouji1"] = "只要客人有意向，无论身在何处都能到达，我是自动手记人偶服务，薇尔莉特·伊芙加登",
	["$vshouji2"] = "初次见面，只要客人有意向，无论身在何处都能到达，我是自动手记人偶服务，薇尔莉特·伊芙加登",
	["$vshouji3"] = "只要客人有意向，无论身在何处都能到达，我是自动手记人偶服务，薇尔莉特·伊芙加登",
	["$vshouji4"] = "丈夫的思念，我会写在信里",
	["$gongqing1"] = "(薇尔莉特，看到了吗？)　嗯，我还是第一次这么看到星星。（不是星星，是彗星！）",
	["$gongqing2"] = "你有看到吗？ 我想我走了三步。",
	["$gongqing3"] = "薇尔莉特，你将不再是道具，而是成为人如其名的人",
	["$gongqing4"] = "不需要传达的信件是没有的哦，大小姐",
	["$gongqing5"] = "不，没有保护好他，很抱歉；让他死了，很抱歉，很抱歉……",
	["$gongqing6"] = "很多的人们的思念，从空中降下。 （薇尔莉特……）少佐……",
	...
	["cv:ZeroTwo"] = "户松遥",
	["xieyu"] = "携驭",
	[":xieyu"] = "①若你有明置男性人物牌：当你使用杀时，可以弃置场上一张延时锦囊牌，增加一名额外目标。②<font color=\"green\"><b>每回合限一次，</b></font>当你使用杀后，若另一张人物牌明置，你可以失去一点体力，视为使用一张不计入次数的杀，然后暗置另一张人物牌直到回合结束。",
	["kuanghe"] = "狂鹤",
	[":kuanghe"] = "进入出牌阶段时，若你无明置男性人物牌且手牌≥体力值：可以弃置所有手牌跳过出牌+弃牌阶段，对所有其他角色各造成一点伤害。当一名角色因此进入濒死状态时，你回复1点体力或摸一张牌。",
	...
	["cv:Asuka"] = "宫村优子",
	["aoshi"] = "傲势",
	[":aoshi"] = "准备阶段开始时，你可以从牌堆顶两张牌中选择一张与一名其他角色拼点：赢，你对其造成1点伤害，若双方点数之差为双数，你可以使用一张拼点牌；未赢，你失去1点体力。",----因此技能拼点时，你点数增加x（x为装备区牌数）。
    ["xinshang"] = "心伤",
	[":xinshang"] = "当你流失体力后，若因此进入濒死状态，将手牌补至5，“傲势”失效直到你下回合结束；若未因此进入濒死状态，可以摸一张牌或使用一张手牌（不计入次数)。",
...
["$AoshiEffect"] = "%from 的拼点牌点数视为 %arg",
["aoshi_null"] = "傲势失效",
["xinshang_draw"] = "摸一张牌",
["xinshang_use"] = "使用一张手牌（不计次数）",
["@xinshang_use"] = "使用一张手牌（不计次数）",
["%Asuka"] = "“我是被挑选出来的人，一个特别的人。”",
...
["Sakunahime"] = "佐久名",
	...
	["xuemai"] = "血脉",
    [":xuemai"] = "锁定技，①当你明置此人物牌时，你随机获得牌堆中的一张【桃】、【酒】或食物牌。②当你使用【桃】、【酒】或食物牌时，你随机获得牌堆中的一张【杀】和【闪】，然后至你的下次回合结束前获得效果（不叠加）：手牌上限+1，出杀额定次数+1。",

	["HatanoKokoro"] = "秦心",
	["@HatanoKokoro"] = "東方project",
	["#HatanoKokoro"] = "情绪丰富的扑克脸",
	["designer:HatanoKokoro"] = "FlameHaze",
	["illustrator:HatanoKokoro"] = "梅まろ",
	["cv:HatanoKokoro"] = "伊濑茉莉也",
	["~HatanoKokoro"] = "这种时候应该带哪一副面具呢",
	["%HatanoKokoro"] = "“嘿哟”",
	
	["mianling"] = "面灵",
	["mianling$"] = "image=image/animate/mianling.png",
	[":mianling"] = "首次明置此人物时，将牌堆首张牌置于人物上作为“面”。出牌阶段限一次，你可以用1张牌替换“面”。根据“面”获得情绪：\n1、♥【怒】出杀次数+1；跳过判定阶段。\n2、♦【喜】摸牌阶段改为获得牌堆2张不同名牌\n3、♣【哀】手牌上限-1\n4、♠【乐】使用的普通杀可变为任意属性",
	["miann"] = "面",
	["mianling_nu"] = "怒",
	["mianling_xi"] = "喜",	
	["mianling_ai"] = "哀",	
   ...
   -- ["mianling_Calm"] = "面灵：移除情绪",
	  ...
	  ["$UseCard_thunder"] = "%from 因 %arg 将 %card 变为雷杀", ["$UseCard_fire"] = "%from 因 %arg 将 %card 变为火杀", ["$UseCard_ice"] = "%from 因 %arg 将 %card 变为冰杀",
	["$mianling1"] = "太子大人是我的恩人！而且也是作为面具的我的母亲....来着？",
	["$mianling2"] = "上吧！六十六张面具们啊！",
	["$mianling3"] = "这个是希望面具。不能碰哦？！",
	["$mianling4"] = "老子是无敌的！！",
	["$mianling5"] = "哈哈哈——呀哈！",
	...
	["cv:Nagisa"] = "中原麻衣",
	["bingruo"] = "病弱",
	[":bingruo"] = "回合开始时，你可以选择一名你对其用“键”为合法目标的其他角色，然后随机将牌堆一张♥牌当作“键”置于其的判定区，然后若你判定区没有键，则你失去1点体力，随机将牌堆一张♥牌当作“键”置于你的判定区；回合结束时，若你判定区有“键”且体力为场上最少之一，你可以选择一名你对其用“键”为合法目标的其他角色，然后随机将牌堆一张♥牌当作“键”置于其的判定区。",
	["yanju"] = "演剧",
	[":yanju"] = "社团技，「光坂高校演剧部」。\n加入条件：当“键”移动到一名其他角色判定区时，你可以询问其是否加入「光坂高校演剧部」。\n社团效果：社团成员的判定区可以放置至多该社团成员数的“键”，社团成员的“键”进行判定时，判定成功条件改为“红色”牌。",
    ["xiyuan"] = "汐愿",
	[":xiyuan"] = "（冈崎朋也&古河渚）连携技，限定技，当你濒死求桃结束时，若你处于濒死状态，你可以将你人物牌中的“古河渚”替换为“冈崎汐”，然后回复1点体力，此阶段至游戏结束人物牌“古河渚”视为游戏内的人物牌。",
	["yanjubu"] = "「光坂高校演剧部」",
	...
	[":dingxin"] = "锁定技，准备阶段开始时，你失去1点体力并随机将一张牌堆一张♥牌当作“键”置于判定区，你的♥手牌不计入手牌上限。当你进入濒死时，你可以依次弃置势力相同角色区域内共计12张♥牌（区域牌可见），然后将体力回复至2，并将此人物牌更换为汐&渚。",
	...
	["@UshioNagisa"] = "Clannad",
	["#UshioNagisa"] = "小镇奇迹",
	["designer:UshioNagisa"] = "clannad最爱",
	["cv:UshioNagisa"] = "兴梠里美&中原麻衣",
	["%UshioNagisa"] = "“与人为善的小镇关爱镇里的居民，这应该是理所当然的事”",
	["kazoku"] = "家族",
	[":kazoku"] = "回合开始时，你可以将牌堆中随机一张♥牌置于一名角色判定区。锁定技，与你势力相同的角色判定区的“键”无数量上限，且“键”判定时生效条件改为“红色”牌。",
	["#KazokuEffect"] = "由于%from 所在的%arg 影响，此次判定成功条件变为“红色牌”。",
  ...
  ["zhilians"] = "秩恋",
  [":zhilians"] = "有此技能的阶段首次明置该人物牌时，你可以令一名其他角色获得“恋”。当你受到伤害时，若场上存在“恋”角色，可以令该角色摸一张牌，然后你失去一点体力并防止此伤害。",
  ...
  ["yixins"] = "遗信",
  [":yixins"] = "当你失去体力或受到伤害后，你可以摸x张牌并可以将至多x张手牌暗置于一名角色人物牌上作为“遗信”（x为你已损失体力值且至少为1）。 ",
  ----[":yixins"] = "当你受到伤害后或进入濒死状态时，摸x张牌并将1~x张牌置于一名其他角色人物牌上作为“遗信”(x为你已损失体力值，至少1），若你进入濒死后未被救回，该角色可以立即获得人物牌上所有“遗信”。<font color=\"orange\">一名其他角色摸牌阶段开始时，获得人物牌上的所有“遗信”，本回合手牌上限+1。</b></font>",
  ["yixins$"] = "image=image/animate/yixins.png",
  ["yixin"] = "遗信",
  ["@yixins"] = "遗信",
  ["~yixins"] = "选择放置的牌和角色",
  ["@yixins_tar"] = "可以选择放置“遗信”的目标",
  --["&yixins"] = "选择1~x张牌作为“遗信”",
  ...
  ["hezous"] = "合奏",
  [":hezous"] = "当你阵亡时，可以令一名其他角色下个回合使用的“遗信”牌不计入额定次数。<font color=\"#C0C0C0\"><b>永久技，</b></font><font color=\"orange\">一名角色摸牌阶段开始，获得人物牌上的所有“遗信”，本回合手牌上限+1。</b></font>",
