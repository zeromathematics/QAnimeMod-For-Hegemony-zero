local hash = {science = {},magic = {},game = {},real = {}, idol = {}}
for _,general in pairs(sgs.Sanguosha:getLimitedGeneralNames())do
	if general:startsWith("lord_") then continue end
	if hash[sgs.Sanguosha:getGeneral(general):getKingdom()] or #sgs.Sanguosha:getGeneral(general):getKingdom():split("|")>1 then
	    local kingdoms = table.Shuffle(sgs.Sanguosha:getGeneral(general):getKingdom():split("|"))
		table.insert(hash[kingdoms[1]],general)
	end
end
...
XmodeRule = sgs.CreateTriggerSkill{
	...
	on_effect = function(self, evnet, room, player, data,ask_who)
		...
		local random_general = getRandomGenerals(9,player:getKingdom(),used)
...
Xmode = {
	...
	on_assign = function(self, room)
		...
		local kingdom = {"science","magic","game","real",}
		local rules_count = {["science"] = 0,["magic"] = 0,["game"] = 0,["real"] = 0}
		for i = 1, 8, 1 do
		...
		for i = 1,8,1 do --开始给每名玩家分配待选择的武将。
			...
			local random_general = getRandomGenerals(9,kingdoms[i],selected)
